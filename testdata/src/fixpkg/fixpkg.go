...
// addressTaken: a variable declared without an initializer whose address is
// handed to another function may be assigned through that pointer, so it is
// not treated as permanently nil.
func addressTaken() {
	var ctx context.Context
	fillCtx(&ctx)
	log.Info().Msg("fix must insert ctx") // want "zerolog event missing .Ctx\\(ctx\\) before Msg\\(\\) - context should be included for proper log correlation"
...
func fillCtx(dst *context.Context) { *dst = context.Background() }
